...
welcome_txt_stim = TextStim(win, text="Welcome to this experiment!", color=(-1, 1, -1), font='Calibri')
...
DIRECTIONS = [0, 90]  # Right (0°) and Left (90°)
    ...
    for direction in DIRECTIONS:
        ...
        for rep in range(0,int(trialn)//10):  
            trial_list.append({
                'coherence': coherence,
                'direction': direction,
                'correct_response': 'left' if direction == 90 else 'right'
            })
...
for trial in trials:
    ...
    # random time between 0.5s and 1.5s
    # uniform distribution: even continuous, equal chance be chosen
    ...
    while response is None:
        ...
        # waitRelease= False tell me as soon as the key goes down.
        # if True, tell me how long people hold the keys
        if keys:
            response = keys[0].name #keys[0] is the very earliest response
            rt = keys[0].rt
